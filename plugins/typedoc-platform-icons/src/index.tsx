import { Application, Comment, DefaultThemeRenderContext, JSX, Reflection } from 'typedoc';

export function load(app: Application): void {

  app.renderer.hooks.on('comment.beforeTags', (ctx: DefaultThemeRenderContext, comment: Comment, reflection: Reflection): JSX.Element => {
    const iosTag = comment.getTag('@ios');
    let result: string = '<div>';

    if (iosTag) {
      iosTag.skipRendering = true;
      result +=
        `<div style="display: inline-flex;align-items: center;border: purple 2px solid;border-radius: 20px;padding: 4px; font-size: small;background-color: purple">
          <svg
            fill="none"
            viewBox="0 0 24 24"
            height="14"
    ...
    const androidTag = comment.getTag('@android');
    if (androidTag) {
      androidTag.skipRendering = true;
      result +=
        `<div style="display: inline-flex;align-items: center;border: green 2px solid;border-radius: 20px;padding: 4px; font-size: small;background-color: green;margin-left: 4px">
