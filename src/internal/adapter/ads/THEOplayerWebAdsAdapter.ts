...
export class THEOplayerWebAdsAdapter implements AdsAPI {
  ...
  currentAdBreak(): Promise<AdBreak> {
    ...
    return adBreak ? Promise.resolve(adBreak as unknown as AdBreak) : Promise.reject<AdBreak>();
  }

  currentAds(): Promise<Ad[]> {
    const ads = this._player.ads?.currentAds;
    return ads ? Promise.resolve(ads as unknown as Ad[]) : Promise.reject<Ad[]>();
  }
  ...
  scheduledAdBreaks(): Promise<AdBreak[]> {
    ...
    return adBreaks ? Promise.resolve(adBreaks as unknown as AdBreak[]) : Promise.reject<AdBreak[]>();
  }
