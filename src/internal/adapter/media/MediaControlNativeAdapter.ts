...
export class MediaControlNativeAdapter implements MediaControlAPI {
  ...
  private readonly _subscription: EmitterSubscription;

  constructor(private readonly _player: THEOplayer) {
    this._subscription = this.mediaControlEmitter.addListener('MediaControlEvent', (event) => {
  ...
  setHandler(action: MediaControlAction, handler: MediaControlHandler | undefined): void {
    if (handler === undefined) {
      ...
      NativeMediaControlModule.removeHandler(this._player.nativeHandle || -1, action);
      return;
    }
